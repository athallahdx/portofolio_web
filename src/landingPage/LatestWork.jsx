...
export default function LatestWork() {
    ...
    return (
        <>
        <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1 }} className='Latest-Work-Container lg:max-w-screen mt-3 lg:mt-6 shadow-top-lg text-white flex-col justify-items-center py-3 md:py-6 lg:py-10 xs:space-y-3 md:space-y-6 lg:space-y-10'>
            ...
            <div className="Latest-Work flex flex-col lg:flex-row justify-items-center gap-10 p-5 w-[95vw]">
                {projects.map((project, index) => (
                    ...
                    key={index} className="relative group h-auto overflow-hidden rounded-lg shadow-lg">
                        <img src={project.img} alt={project.title} className="w-auto lg:h-[35vh] object-cover transition-transform duration-500 group-hover:scale-110" />
                        ...
                        <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                            <h3 className="text-3xl text-center xs:text-xl md:text-4xl lg:text-xl 2xl:text-3xl font-bold mb-2">{project.title}</h3>
                            <p className='hidden xs:block xs:text-x md:text-xl mdp:text-2xl lg:text-base 2xl:text-lg text-center px-5'>{project.description}</p>
                            <a href={project.link} className="hidden sm:block bg-white md:mt-8 mdp:mt-8 lg:mt-5 2xl:mt-10 text-black px-4 py-2 rounded-lg hover:bg-gray-300 text-2xl transition">
